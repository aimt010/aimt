...
fn run_install_kilo(dir: &Path) -> std::process::Output {
...
#[test]
fn install_kilo_succeeds() {
    let dir = tempfile_dir();
    let out = run_install_kilo(&dir);
    assert!(
        out.status.success(),
        "install kilo should succeed: stdout={}, stderr={}",
...
#[test]
fn install_kilo_skill_has_frontmatter() {
    let dir = tempfile_dir();
    let out = run_install_kilo(&dir);
    assert!(out.status.success());
    let content = std::fs::read_to_string(dir.join(".kilo/skills/aimt/SKILL.md")).unwrap();
...
#[test]
fn install_kilo_instructions_present() {
    let dir = tempfile_dir();
    let out = run_install_kilo(&dir);
    assert!(out.status.success());
    let skill = std::fs::read_to_string(dir.join(".kilo/skills/aimt/SKILL.md")).unwrap();
    assert!(
...
#[test]
fn install_kilo_preserves_existing_skills() {
    let dir = tempfile_dir();
    let other_skill = dir
        .join(".kilo")
        .join("skills")
...
#[test]
fn install_kilo_preserves_existing_agents_skills() {
    let dir = tempfile_dir();
    let other_skill = dir
        .join(".agents")
        .join("skills")
...
#[test]
fn install_kilo_idempotent() {
    let dir = tempfile_dir();
    let out1 = run_install_kilo(&dir);
    assert!(out1.status.success());
    let skill1 = std::fs::read_to_string(dir.join(".kilo/skills/aimt/SKILL.md")).unwrap();
    let out2 = run_install_kilo(&dir);
    assert!(out2.status.success(), "second install should succeed");
    let skill2 = std::fs::read_to_string(dir.join(".kilo/skills/aimt/SKILL.md")).unwrap();
...
#[test]
fn install_kilo_no_duplicate_content() {
    let dir = tempfile_dir();
    let out1 = run_install_kilo(&dir);
    assert!(out1.status.success());
    let out2 = run_install_kilo(&dir);
...
#[test]
fn install_kilo_invalid_project() {
    let dir = std::env::temp_dir().join(format!(
        "aimt_kilo_nonexistent_{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos()
...
#[test]
fn install_kilo_preserves_unrelated_files() {
    let dir = tempfile_dir();
...
#[test]
fn install_kilo_does_not_affect_other_tools() {
    let dir = tempfile_dir();
    let out_kilo = run_install_kilo(&dir);
    assert!(out_kilo.status.success());
    assert!(dir.join(".kilo/skills/aimt/SKILL.md").is_file());
    ...
    assert!(
        ...
        "kilo should not create CLAUDE.md by default"
    );
    // Install opencode afterwards should preserve kilo skill
    let out_opencode = {
        std::process::Command::new("cargo")
            .args([
                "run",
